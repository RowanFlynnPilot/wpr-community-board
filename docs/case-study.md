...
The board ships speaking three languages — English, Español, and Hmoob
(Wausau has one of the largest Hmong communities per capita in the U.S.),
with the non-English strings awaiting native-speaker review before the
public announcement — and the analytics record which language each session
used, so the translation work shows up as a measured outcome in the grant
report. The desk takes dictation: a note phoned in by a neighbor who
doesn't do web forms gets typed up by the editor and enters through the
same single submission path as everything else. The weekly newsletter
block is generated by one button on the editor's desk, deep links
included. And a print stylesheet turns the live board into a clean
two-column "From the Community Board" page for the paper — closing the
loop back to the corkboard the design came from.
